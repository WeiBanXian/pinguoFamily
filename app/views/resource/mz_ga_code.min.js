//***********************************************************
// Name: Chevy Mobile miaozhen与ga整合一起的 v 2.4
...
(function(hy){
	...
	function trackPV (mzPage,gaPage){
	    ...
	    if(gaPage && gaPage!=""){
	    	...
	    	ga('send', 'pageview', pp);_sgma("send","pageview", {url:pp});_utaq.push(['trackPageView',pp]);
	    }
	...
	function trackEvent(mzBtn,category, action, label){
    	...
    	//GA
        if(label && label!=""){
        	ga('send', 'event', category, action, label);_utaq.push(['trackEvent', category, action, label]);//_sgma("send","event", category, action, label);// todo, next version
        }
	...
	hy.sgmaPrice = _sgmaPrice;//手机没发现这个要求
